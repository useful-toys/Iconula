...
| [MDR 0003](0003-gravacao-agregada-da-colecao.md) | Gravação agregada da coleção | Aceito | firestore, persistencia | Três funções de escrita (`merge: true`, `mergeFields`, atestação); debounce ~4s, teto ~20s, flush em `pagehide`, cache IndexedDB; proteção de corrida, descartar pendências. |
...
| [MDR 0007](0007-persistencia-no-armazenamento-local.md) | Persistência no armazenamento local | Aceito | persistencia, localStorage, IndexedDB, consentimento | Preferências de vista, colapso manual e consentimento de analytics no `localStorage` (versionado, por dispositivo); cache do Firestore no IndexedDB (multi-aba, flush garantido). |
