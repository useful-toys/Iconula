...
| [ADR 0009](0009-testes-co-localizados.md) | Testes co-localizados | Aceito | testes, arquitetura | Cada módulo ou componente tem seu arquivo de teste co-localizado (`.test.js` ou `.test.jsx`), no mesmo diretório. Segunda exceção: `e2e/` para os testes E2E (ADR 0010). |
| [ADR 0010](0010-testes-e2e-com-playwright-e-emuladores-do-firebase.md) | Testes E2E com Playwright e emuladores do Firebase | Aceito | testes, auth | Playwright contra os emuladores de Auth e Firestore (`demo-iconula`); login duplo (popup fake do Google para telas de login, e-mail/senha instantâneo para o resto) e fixture de coleção; `e2e/` fora da co-localização. |
| [ADR 0011](0011-analytics-de-uso-com-google-analytics-4.md) | Analytics de uso com Google Analytics 4, entregue por gtag sob consentimento | Aceito | analytics, privacidade, integracao | GA4 linkado ao projeto, região Brasil, entregue por `gtag.js` carregado por um módulo mínimo após consentimento — sem SDK `firebase/analytics` nem snippet CDN do console; Measurement ID via env. |
