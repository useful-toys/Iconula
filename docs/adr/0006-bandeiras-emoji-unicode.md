...
# ADR 0006: Bandeiras como emoji Unicode via Twemoji

## Status

Aceito (revisado após teste manual; revisado novamente para remover a dependência de CDN em runtime)

## Contexto

- Bandeira de cada uma das 48 seleções — opções avaliadas: emoji Unicode nativo, CDN de imagens (ex.: flagcdn.com), SVG locais.
- Decisão inicial: emoji Unicode nativo (`"🇦🇷"` etc.) — sem dependência externa nem assets no repositório.
- **Problema em teste manual**: no Windows, a fonte de emoji do sistema (Segoe UI Emoji) não tem glifos coloridos de bandeira — cai no fallback de mostrar o código ISO como texto ("AR" em vez de 🇦🇷). Afeta as 48 bandeiras para qualquer usuário Windows sem fonte de emoji colorida adicional, não é caso isolado.
