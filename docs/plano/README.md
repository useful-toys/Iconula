...
| 36 | [Analytics de uso com Google Analytics 4 e consentimento](0036-analytics-de-uso/) | Medir visitas, sessões e engajamento com GA4 (região Brasil), carregado por um módulo mínimo sob consentimento, com banner nas duas telas e política/conformidade atualizadas | — | `feat: analytics de uso (GA4) com banner de consentimento` | Em andamento |
...
| 0002 | [Funções de exclusão nas duas camadas](0031-exclusao-de-dados-e-conformidade-da-politica/0002-funcoes-de-exclusao-nas-duas-camadas.md) | Apagar documento em `colecaoRemota`; reautenticar e apagar conta em `firebase`. | Concluída |
...
| 0003 | [Tooltip do desfazer com código e operação](0033-sobre-tooltip-do-desfazer-e-indicador-de-pendencia/0003-tooltip-do-desfazer-com-codigo-e-operacao.md) | `Desfazer: +1 em BRA05`, tooltip no padrão IDR 0048 e `aria-label` sincronizado. | Concluída |
| 0004 | [Indicador "não salvo" no título](0033-sobre-tooltip-do-desfazer-e-indicador-de-pendencia/0004-indicador-nao-salvo-no-titulo.md) | O relógio dá lugar a "não salvo" enquanto há alteração não gravada. | Concluída |
| 0005 | [Debounce e teto de gravação maiores](0033-sobre-tooltip-do-desfazer-e-indicador-de-pendencia/0005-debounce-e-teto-de-gravacao-maiores.md) | `DEBOUNCE_MS` 2000→4000, `TETO_MS` 10000→20000 em `gravacaoAgregada.js`. | Concluída |
...
Decisões do esmiuçamento ([PR #86](https://github.com/useful-toys/Iconula/pull/86)):
QR code Pix estático para doação, com valor sugerido de R$5,00 editável,
chave aleatória do mantenedor e dados do recebedor no payload BR Code
([TDR 0029](../tdr/0029-geracao-local-do-qr-code-pix.md)); vista dedicada
"Apoie o projeto" (molde de vista interna sem router), acessível pelo
rodapé das duas telas e por um bloco próprio no menu de ações, depois do
bloco "Sobre" da Fase 33
([IDR 0070](../idr/0070-apoio-ao-projeto-rodape-menu-e-vista-com-qr-pix.md)).
Depende da Fase 33 entregue: as duas fases tocam os mesmos arquivos
(`MenuDeAcoes.jsx`, `Rodape.jsx`, `TelaDeLogin.jsx`, `App.jsx`).
`docs/requisitos.md` § Apoio ao projeto já foi criada no PR do
esmiuçamento.
