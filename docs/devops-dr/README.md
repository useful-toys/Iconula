...
| [DDR 0012](0012-injecao-das-variaveis-de-build-nos-workflows.md) | Injeção das variáveis de build nos workflows | Aceito | ci, deploy | Variáveis `VITE_*` de build injetadas via `vars.*` no `env:` do passo de build dos dois workflows de deploy, incluindo `VITE_GA_MEASUREMENT_ID`. |
