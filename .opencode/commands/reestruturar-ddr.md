---
description: Reestrutura os DDRs (docs/devops-dr/): funde duplicatas, resolve contradições e revisa a redação
---

<!-- Copyright (c) 2026 Daniel Felix Ferber -->

Reorganize os **DDRs** (`docs/devops-dr/`): entenda todas as decisões, elimine
...
Foco opcional: se `$ARGUMENTS` não vier vazio (ex.: `0001-0005` ou `deploy`),
...
1. Leia, no início, tudo o que já existe: `AGENTS.md`; o guia
   `docs/devops-dr/CLAUDE.md` (estrutura, regras de redação e atualização do
   índice); o documento vivo `docs/devops.md`; **todos** os DDRs de
   `docs/devops-dr/` e o índice `docs/devops-dr/README.md`. Se algum desses
...
5. Crie a worktree de trabalho a partir da branch original, com nome sugerido
   pelas skills `git-branch-name` (tipo/assunto `docs-reestruturar-ddr`):
   `git worktree add .worktrees/<nome> -b <nome> <branch-original>`.
   Trabalhe **somente** dentro dela.

## 2. Diagnóstico (somente leitura)
Monte o entendimento de cada decisão — título, status, assunto de DevOps
(CI/CD, deploy, branch protection, secrets, ambientes), posição cronológica
(datas/ordem no `## Histórico`) e referências cruzadas. Depois aponte:
...
- **tipo fora do lugar** — o registro está numa pasta cujo tipo não corresponde
  ao assunto (ex.: um DDR sobre arquitetura ou interface); aplique as regras de
...
- **redação** — desvios das regras de estilo do `AGENTS.md` e do § Estilo de
  prosa do `docs/devops-dr/CLAUDE.md`.
...
- mudanças no índice `docs/devops-dr/README.md` e no documento vivo
  `docs/devops.md`.
...
- Siga o `docs/devops-dr/CLAUDE.md` para redação, estrutura e atualização do
  `docs/devops-dr/README.md` (tabela `Nº | Título | Status | Tags | Resumo`, no
  mesmo commit).
...
- Atualize `docs/devops.md` (documento vivo) para refletir a situação atual.
