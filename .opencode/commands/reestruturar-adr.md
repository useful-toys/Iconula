---
description: Reestrutura os ADRs (docs/adr/): funde duplicatas, resolve contradições e revisa a redação
---

<!-- Copyright (c) 2026 Daniel Felix Ferber -->

Reorganize os **ADRs** (`docs/adr/`): entenda todas as decisões, elimine
...
Foco opcional: se `$ARGUMENTS` não vier vazio (ex.: `0003-0005` ou `firestore`),
...
1. Leia, no início, tudo o que já existe: `AGENTS.md`; o guia
   `docs/adr/CLAUDE.md` (estrutura, regras de redação e atualização do índice);
   o documento vivo `docs/arquitetura.md`; **todos** os ADRs de `docs/adr/` e o
   índice `docs/adr/README.md`. Se algum desses arquivos não existir, pare e diga
...
5. Crie a worktree de trabalho a partir da branch original, com nome sugerido
   pelas skills `git-branch-name` (tipo/assunto `docs-reestruturar-adr`):
   `git worktree add .worktrees/<nome> -b <nome> <branch-original>`.
   Trabalhe **somente** dentro dela.

## 2. Diagnóstico (somente leitura)
Monte o entendimento de cada decisão — título, status, assunto, posição
...
- **tipo fora do lugar** — o registro está numa pasta cujo tipo não corresponde
  ao assunto (ex.: um ADR sobre implementação interna ou interface); aplique as
...
- mudanças no índice `docs/adr/README.md` e no documento vivo
  `docs/arquitetura.md`.
...
- Siga o `docs/adr/CLAUDE.md` para redação, estrutura e atualização do
  `docs/adr/README.md` (tabela `Nº | Título | Status | Tags | Resumo`, no mesmo
  commit).
