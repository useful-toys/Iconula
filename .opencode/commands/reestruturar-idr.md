---
description: Reestrutura os IDRs (docs/idr/): funde duplicatas, resolve contradições e revisa a redação
---

<!-- Copyright (c) 2026 Daniel Felix Ferber -->

Reorganize os **IDRs** (`docs/idr/`): entenda todas as decisões, elimine
...
Foco opcional: se `$ARGUMENTS` não vier vazio (ex.: `0001-0010` ou `foco`),
...
1. Leia, no início, tudo o que já existe: `AGENTS.md`; o guia
   `docs/idr/CLAUDE.md` (estrutura, regras de redação e atualização do índice);
   os documentos vivos `docs/interface.md` e `docs/requisitos.md`; **todos** os
   IDRs de `docs/idr/` e o índice `docs/idr/README.md`. Se algum desses arquivos
   não existir, pare e diga o que faltou — não invente formato nem índice.
...
5. Crie a worktree de trabalho a partir da branch original, com nome sugerido
   pelas skills `git-branch-name` (tipo/assunto `docs-reestruturar-idr`):
   `git worktree add .worktrees/<nome> -b <nome> <branch-original>`.
   Trabalhe **somente** dentro dela.

## 2. Diagnóstico (somente leitura)
Monte o entendimento de cada decisão — título, status, assunto de interface,
posição cronológica (datas/ordem no `## Histórico`) e referências cruzadas.
Depois aponte:
...
- **contradição** — decisões incompatíveis entre si; em conflito com
  `docs/requisitos.md`, requisitos vencem;
...
- **tipo fora do lugar** — o registro está numa pasta cujo tipo não corresponde
  ao assunto (ex.: um IDR sobre implementação interna ou DevOps); aplique as
...
- mudanças no índice `docs/idr/README.md` e no documento vivo
  `docs/interface.md`.
...
- Siga o `docs/idr/CLAUDE.md` para redação, estrutura e atualização do
  `docs/idr/README.md` (tabela `Nº | Título | Status | Tags | Resumo`, no mesmo
  commit).
