...
1. **Delegar** com a ferramenta `task`, agente `general` e esperar terminar. Prompt:
   ...
   ```
   Execute a Tarefa NNNN-XXXX seguindo .opencode/commands/executar-tarefa.md,
...
2. Renumere registros que colidem com a `main`:
   ...
   3. Novo número = próximo livre após o maior entre `origin/main` e a branch,
      ...
      referência antiga.
   4. Commit próprio — mensagem pelo guia § Convenções de Git › Commit —; guarde a tabela `antigo → novo`.
