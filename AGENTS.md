...
| `src/components/MenuDeCompartilhar.jsx` | Botão compartilhar à esquerda do avatar e o popup das listas de troca, com as duas cópias separadas por filete, onde o navegador oferece a folha do sistema o item de compartilhar logo abaixo de cada cópia, e um terceiro bloco com a chave `Link do catálogo: ligado/desligado`, que grava na hora sem fechar o popup (ver [IDR 0024](docs/idr/0024-acoes-raras-em-menu-do-cabecalho.md), [IDR 0055](docs/idr/0055-catalogo-compartilhado-por-link-somente-leitura.md) e [IDR 0018](docs/idr/0018-usuario-especialista-e-minimalismo.md)). |
...
| `e2e/` | Testes E2E (Playwright), contra os emuladores Auth+Firestore do Firebase — segunda exceção à co-localização ([ADR 0009](docs/adr/0009-testes-co-localizados.md), [ADR 0010](docs/adr/0010-testes-e2e-com-playwright-e-emuladores-do-firebase.md)). `e2e/helpers/login.js` (login duplo: popup fake do Google e e-mail/senha instantâneo) e `e2e/helpers/fixture.js` (grava um estado conhecido no Firestore Emulator) dão suporte a `e2e/catalogo.spec.js`, o teste de fumaça. `playwright.config.js` (padrão, build+preview) e `playwright.dev.config.js` (atalho, dev server) configuram os dois scripts `npm run test:e2e`/`test:e2e:dev` — ver [docs/teste-e2e.md](docs/teste-e2e.md). |
