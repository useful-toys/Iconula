...
| `src/components/ContaApagada.jsx` | Tela final da exclusão de dados, em branco — título "Conta apagada", uma linha curta e o botão "Voltar à tela de login" —, montada antes da guarda de login, sem router (ver [IDR 0060](docs/idr/0060-apagar-meus-dados-na-politica-em-dois-passos.md) e [TDR 0020](docs/tdr/0020-privacidade-como-vista-interna.md)). |
...
| `src/components/Estatisticas.jsx` | Vista interna de estatísticas da coleção, sem router: cinco blocos (resumo, progresso por grupo e por seção, repetidas por seção e histograma) derivados da coleção em memória, somente leitura, com gráficos à mão em SVG/CSS e as cores da página principal — estado, grupo e seleção (ver [IDR 0072](docs/idr/0072-pagina-de-estatisticas-como-vista-interna.md) e [TDR 0030](docs/tdr/0030-graficos-de-estatisticas-a-mao-sem-biblioteca.md)). |
...
| `src/lib/corDoSelo.js` | Cor do selo `×N` por unidades sobrando, do laranja ao vermelho (ver [IDR 0066](docs/idr/0066-gradiente-de-cor-do-selo-conforme-sobrando.md)); compartilhada pelo cartão (`Figurinha.jsx`) e pelo histograma das estatísticas. |
| `src/lib/estatisticas.js` | Derivação pura dos cinco blocos da página de estatísticas (resumo, progresso por grupo da Copa e por seção, repetidas por seção e histograma), em memória e sem dependência de React; lê só `calcularPlacar` de `progresso.js` (ver [IDR 0072](docs/idr/0072-pagina-de-estatisticas-como-vista-interna.md) e [TDR 0030](docs/tdr/0030-graficos-de-estatisticas-a-mao-sem-biblioteca.md)). |
...
| `src/lib/versoesDosTextos.js` | Versões publicadas dos textos de conformidade (termos de uso e política de privacidade): data de vigência em ISO, **único** lugar do código onde as versões vivem, com a regra de que só mudança material as altera (ver [MDR 0009](docs/model-dr/0009-campos-de-aceite-dos-textos.md) e [IDR 0062](docs/idr/0062-reaceite-reusa-a-tela-de-atestacao.md)). |
